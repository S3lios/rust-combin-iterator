...
impl<'a, A> VecAltern<'a, A> {
    ...
    pub fn add(mut self, iterator: impl Iterator<Item = &'a A> + 'a) -> VecAltern<'a, A> {
...
mod tests {
    ...
    fn altern() {
        ...
        let iter = VecAltern::new().add(vec1.iter()).add(vec2.iter()).add(vec3.iter());
